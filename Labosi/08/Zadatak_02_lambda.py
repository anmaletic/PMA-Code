...
def InitializeMainWindow():
    ...
    buttonPretrazi = ttk.Button(width="39", text="Pretraži", command=lambda: poruka.set(PretraziCommand(unos.get(), target.get())))
...
def PretraziCommand(unos, target):
    if len(unos) == 0:
        _poruka = "Lista je prazna."
    else:
        if len(target) == 0:
            _poruka = "Nije upisana tražena vrijednost."
        else:
            listaUnosa = unos.replace(" ", "").split(",")
            ...
            if listaUnosa == listaUnosaSorted:
                ...
                _result = BinarySearch(listaUnosa, int(target), _low, _high)

                if _result:
                    _poruka = "Postoji tražena vrijednost u listi."
                else:
                    _poruka = "Ne postoji tražena vrijednost u listi."
            else:
                _poruka = "Lista nije sortirana."
    return _poruka
